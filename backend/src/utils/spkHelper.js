...
// Hitung bobot AHP dengan metode Rata-rata Kolom Ternormalisasi (Sesuai Frontend)
const hitungBobotAHP = (matrix) => {
  ...
  // 2. Matriks Ternormalisasi & Hitung Eigen Vektor (Nilai Baris / n)
  const weights = new Array(n).fill(0);
  for (let i = 0; i < n; i++) {
    let rowSum = 0;
    for (let j = 0; j < n; j++) {
      rowSum += A[i][j] / colSum[j];
    }
    weights[i] = rowSum / n;
  }
