...
// Helper: ambil gambar_list ("Gambar Wisata") untuk array id wisata
...
module.exports = {
  ...
  createAlternatif: async (req, res) => {
    try {
      ...
      // Maksimal 1 foto: ambil hanya file pertama
      const files = (req.files && req.files.gambar_list) || [];
      const file = files[0] || null;
      const gambarUtama = file ? file.filename : null;
      ...
      // Simpan 1 gambar ke wisata_gambar
      if (file) {
        await db('wisata_gambar').insert({
          id_alternatif: newId,
  ...
  updateAlternatif: async (req, res) => {
    try {
      ...
      // Jika ada foto baru -> GANTI: hapus semua gambar lama, simpan 1 yang baru
      if (file) {
        await db('wisata_gambar').where('id_alternatif', id).del();
        await db('wisata_gambar').insert({
          id_alternatif: id,
          nama_file: file.filename,
          urutan: 0,
        ...
        updateData.gambar = file.filename;
      }
  ...
  deleteAlternatif: async (req, res) => {
    try {
      ...
      // wisata_gambar terhapus otomatis via CASCADE
  ...
  // [DELETE] HAPUS 1 GAMBAR WISATA SPESIFIK
