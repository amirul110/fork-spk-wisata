  ...
  hitungRekomendasi: async (req, res) => {
    ...
    try {
      ...
      if (!ahp.konsisten) {
        ...
        return res.status(400).json({
          status: API_STATUS.BAD_REQUEST,
          message: `Perbandingan kriteria tidak konsisten (CR = ${ahp.CR.toFixed(8)} >= 0.1). Silakan isi ulang preferensi Anda.`,
          data: { cr: Number(ahp.CR.toFixed(8)) }
        });
      }
      ...
      // --- SIMPAN RIWAYAT USER + SNAPSHOT BOBOT AHP ---
      const bobotUserSnapshot = {
        metode: "AHP-dinamis (pairwise)",
        matrix,
        cr: Number(ahp.CR.toFixed(8)),
        bobot_user: kriteriaIds.map((id, index) => ({
          id_kriteria: id,
          bobot: Number((ahp.weights[index] || 0).toFixed(8)),
        })),
      };
      ...
      await trx(TABLES.RIWAYAT_PENCARIAN).insert({
        id_wisatawan: userId,
        detail_pencarian: JSON.stringify({ userLocation, matrix, bobot_user: bobotUserSnapshot, cr: Number(ahp.CR.toFixed(8)) }),
        created_at: new Date()
      });
      ...
      const finalResult = candidates.map(item => {
        ...
        kriteriaIds.forEach((kID) => {
          ...
          detailKriteria.push({
            ...
            bobot: Number(bobot.toFixed(8)),
            kontribusi: Number(kontribusi.toFixed(8)),
          });
        });

        return {
          ...item,
          skor_rekomendasi: skorAkhir.toFixed(8),
      ...
      const dataToInsert = top5.map((item, index) => ({
        ...
        skor_rekomendasi: Number(item.skor_rekomendasi),
        ranking: index + 1,
      ...
      return res.json({
        ...
        data: {
          ...
          cr: Number(ahp.CR.toFixed(8)),
          lambda_max: Number(ahp.lambdaMax.toFixed(8)),
          ci: Number(ahp.CI.toFixed(8)),
          bobot_ahp: kriteriaIds.map((id) => {
            ...
            return {
              ...
              bobot: Number((weightByKriteriaId[id] || 0).toFixed(8)),
            };
          }),
    ...
    try {
      ...
      const data = await db(TABLES.HASIL_REKOMENDASI)
        ...
        .select(
          ...
          'hasil_rekomendasi.skor_rekomendasi as skor_akhir',
      ...
      const formattedData = data.map(item => {
        ...
        return {
          ...
          skor: parseFloat(item.skor_akhir || 0).toFixed(8),
    ...
    try {
      ...
      const data = await db(TABLES.HASIL_REKOMENDASI)
        ...
        .select(
          ...
          db.raw(`AVG(${TABLES.HASIL_REKOMENDASI}.skor_rekomendasi) as rata_rata_skor`),
      ...
      const formattedData = data.map((item, index) => ({
        ...
        nama_wisata: item.nama_wisata,
        skor_rata_rata: parseFloat(item.rata_rata_skor).toFixed(8),
