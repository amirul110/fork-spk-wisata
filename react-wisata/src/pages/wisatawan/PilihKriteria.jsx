  ...
  return (
    <div className="p-3" style={{ maxWidth: 960, margin: "0 auto" }}>
      ...
      >
        ...
        {/* LANGKAH 2 */}
        <h3 className="text-lg font-bold text-800 mt-0 mb-2">
          Langkah 2 — Matriks Perbandingan (4 desimal)
        </h3>
        <p className="text-600 text-sm mt-0 mb-3">
          ...
          <tbody>
            {KRITERIA_AHP.map((k, i) => (
              <tr key={k.id}>
                ...
                <td style={tdStyle}>{fmt(detailAhp.rowSums[i])}</td>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "1rem" }}>
          ...
          <tbody>
            {KRITERIA_AHP.map((k, j) => (
              <tr key={k.id}>
                <td style={{ ...tdStyle, textAlign: "left" }}>{k.nama}</td>
                <td style={tdStyle}>{fmt(detailAhp.colSum[j])}</td>
                <td style={tdStyle}>{fmt(ahp.weights[j])}</td>
                <td style={tdStyle}>
                  {fmt(detailAhp.colSum[j] * ahp.weights[j])}
                </td>
              </tr>
            ))}
            <tr>
        ...
        >
          <div className="mb-2">
            <b>n</b> = {detailAhp.n} &nbsp;&nbsp;|&nbsp;&nbsp;{" "}
            <b>λ max</b> = {fmt(ahp.lambdaMax)} &nbsp;&nbsp;|&nbsp;&nbsp;{" "}
            <b>RI</b> (n = {detailAhp.n}) = {fmt(detailAhp.RI, 2)}
          </div>

          <div className="mb-2">
            <b>CI</b> = (λ max − n) / (n − 1) ={" "}
            <b>({fmt(ahp.lambdaMax)} − {detailAhp.n})</b> /{" "}
            <b>({detailAhp.n} − 1)</b> ={" "}
            <b>{fmt(ahp.lambdaMax - detailAhp.n)} / {detailAhp.n - 1}</b>{" "}
            = <b style={{ color: "#1d4ed8" }}>{fmt(ahp.CI)}</b>
          </div>

          <div className="mb-2">
            <b>CR</b> = CI / RI ={" "}
            <b>{fmt(ahp.CI)} / {fmt(detailAhp.RI, 2)}</b> ={" "}
            <b style={{ color: "#1d4ed8" }}>{fmt(ahp.CR)}</b>
          </div>

          <div>
            Karena <b>CR = {fmt(ahp.CR)} {konsisten ? "<" : "≥"} 0.1</b>,
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "0.5rem" }}>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td style={{ ...tdStyle, fontWeight: 700 }}>
                {fmt(ahp.weights.reduce((a, b) => a + (b || 0), 0))}
              </td>
